...
export function AuthModal({ isOpen, onClose, initialMode = 'signup', urlToShorten = '' }) {
  ...
  const handleGoogleLogin = () => {
    ...
    if (!clientId || clientId.includes('your-google-client-id')) {
      setAuthError('Google Client ID not configured yet in .env');
      return;
    }
      ...
      try {
        const client = window.google.accounts.oauth2.initTokenClient({
          ...
          error_callback: (err) => {
            ...
            setAuthError('Google Sign-In was cancelled or encountered an issue.');
          }
        });
        client.requestAccessToken();
      } catch (e) {
        console.error('Failed to initialize Google OAuth:', e);
        setAuthError('Could not open Google popup. Check browser popup permissions.');
      }
    } else {
      setAuthError('Google Identity Services library is still loading. Please try again.');
    }
  };
        ...
        <div className="auth-modal-body">
          {userProfile ? (
            <div className="auth-success-state">
              <CheckCircle size={42} color="#22c55e" style={{ margin: '0 auto 12px auto' }} />
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.2rem', marginBottom: '6px' }}>
                Welcome, {userProfile.name || userProfile.email}!
              </h3>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
                Authenticated with Google. Redirecting to SHRNK...
              </p>
            </div>
          ) : (
            <>
